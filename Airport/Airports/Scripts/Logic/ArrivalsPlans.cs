...
namespace Airports
{
    class ArrivalsPlans : LineInfo
    {
        public ArrivalsPlans() { }
        public ArrivalsPlans(DateTime time, string city, Number number, Status status, Terminal terminal, Class cLass, List<Passenger> passengers)
        {
            this.Class = cLass;
        ...
        public override void Show(List<LineInfo> listinfo)
        {
            ...
            {
                if (i.Info == Info.Arrival)
                {
                    Console.WriteLine("{0,25}  #{1}", " Arrival ", index);
        ...
        public override void Create(List<LineInfo> info)
        {
            ...
            info.Add(new ArrivalsPlans(inputTime, inputCity, inputNumber, inputStatus, inputTerminal, inputcLass, Passenger.AllPassengers));
        }
        ...
        public override void Edit(List<LineInfo> info)
        {
            ...
            else { info[num - 1].Status = inputStatus; }
            Console.WriteLine("Of which terminal is the flight? 1) {0} 2) {1} 3) {2} 4) {3} 5) {4} 6) {5} ", Terminal.first, Terminal.second, Terminal.third, Terminal.fourth, Terminal.fifth, Terminal.sixth);
            check = Enum.TryParse(Console.ReadLine(), out inputTerminal);
            if (!check)
            {
